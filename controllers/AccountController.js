...
router.get('/:publicKey', function (req, res) {
    db.getAccountInfo(req.params.publicKey)
        .then(async function (datasnapshot) {
            ...
            for (let key in jsonRes.interact) {
                sequence++;
            }
            ...
            if (jsonRes.follow) {
                ...
                for (let key in jsonRes.follow) {
                    sequence++;
                    if (jsonRes.follow[key].block > block_newest_follow) {
                        block_newest_follow = jsonRes.follow[key].block;
                        info.follow = jsonRes.follow[key].users;
                    }
                }
            }

            let followed = await db.getAccountFollowed(req.params.publicKey);

            let jsonFollowed = {};

            if (followed.toJSON()) {
                jsonFollowed = followed.toJSON();
            }

            info.followed = jsonFollowed;
